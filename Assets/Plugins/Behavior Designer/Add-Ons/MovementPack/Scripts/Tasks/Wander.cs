...
namespace Opsive.BehaviorDesigner.AddOns.MovementPack.Runtime.Tasks
{
    ...
    public class Wander : MovementBase
    {
        ...
        [Tooltip("Amount to increase the degrees if a wander position can't be found.")]
        ...
        private bool TrySetDestination()
        {
            ...
            var attempts = m_DestinationRetries.Value;
            Vector3 destination;
            while (attempts > 0) {
                ...
                destination = transform.position + direction.normalized * m_WanderDistance.Value.RandomValue;
